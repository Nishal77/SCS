-- ========================================
-- Complete Order Management System Setup
-- ========================================
...
-- ========================================
-- 4. Create Customer Feedback Table
-- ========================================
...
-- ========================================
-- 6. Create Functions for Auto-Generation
-- ========================================
...
-- Insert sample transaction (uncomment and modify if needed for testing)
/*
...
COMMENT ON COLUMN order_items.transaction_id IS 'Reference to the main transaction';
COMMENT ON COLUMN order_items.product_id IS 'Reference to the inventory item';
...
-- Function to get order items for a transaction
CREATE OR REPLACE FUNCTION get_order_items(transaction_uuid UUID)
RETURNS TABLE (
    id UUID,
    item_name TEXT,
    item_description TEXT,
...
BEGIN
    ...
    SELECT 
        ...
        oi.special_requests
    FROM order_items oi
    WHERE oi.transaction_id = transaction_uuid
    ORDER BY oi.created_at;
END;
$$ LANGUAGE plpgsql;

-- Function to get order status history
CREATE OR REPLACE FUNCTION get_order_status_history(transaction_uuid UUID)
RETURNS TABLE (
    status TEXT,
    status_message TEXT,
...
BEGIN
    ...
    WHERE osh.transaction_id = transaction_uuid
    ORDER BY osh.updated_at DESC;
END;
$$ LANGUAGE plpgsql;

-- Function to get payment history
CREATE OR REPLACE FUNCTION get_payment_history(transaction_uuid UUID)
RETURNS TABLE (
    payment_status TEXT,
    payment_method TEXT,
    amount NUMERIC,
...
BEGIN
    ...
    WHERE ph.transaction_id = transaction_uuid
